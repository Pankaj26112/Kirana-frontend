...
export default function App() {
  ...
  const loadProducts = async () => {
    try {
      const data = await api("/api/products", {}, currentUser?.token || getToken());
      setProducts(data.products || []);
    } catch (e) { showToast("Products load nahi hue!", "error"); }
  };

  const loadAds = async () => {
    try {
      const data = await api("/api/ads/active", {}, currentUser?.token || getToken());
      setAds(data.ads || []);
    } catch { /* silent */ }
  };
  ...
  const loadAllAds = async () => {
    try {
      const data = await api("/api/ads", {}, currentUser?.token || getToken());
      setAds(data.ads || []);
    } catch { /* silent */ }
  };
  ...
  const loadMyOrders = async () => {
    try {
      const data = await api("/api/orders/my", {}, currentUser?.token || getToken());
  ...
  const loadAllOrders = async () => {
    try {
      const data = await api("/api/orders", {}, currentUser?.token || getToken());
      setOrders(data.orders || []);
    } catch { /* silent */ }
  };
  ...
  const handleRegisterFinal = async (name, phone, password, tempToken) => {
    ...
    try {
      const data = await api("/api/auth/register", {
        method: "POST",
        body: JSON.stringify({ name, phone, password, temp_token: tempToken }),
      });
      setToken(data.token);
      setCurrentUser({ ...data.user, token: data.token });
      setPage("shop");
  ...
  const saveProduct = async (prod) => {
    ...
    try {
      if (prod.id) {
        await api(`/api/products/${prod.id}`, { method: "PUT", body: JSON.stringify(prod) }, currentUser.token);
        showToast("✅ Product update ho gaya!");
      } else {
        await api("/api/products", { method: "POST", body: JSON.stringify(prod) }, currentUser.token);
  ...
  const deleteProduct = async (id) => {
    setLoading(true);
    try {
      await api(`/api/products/${id}`, { method: "DELETE" }, currentUser.token);
      await loadProducts();
      showToast("🗑️ Product hata diya!");
      setModal(null);
    } catch (e) {
      showToast(e.message, "error");
    } finally {
  ...
  const saveAd = async (ad) => {
    ...
    try {
      if (ad.id) {
        await api(`/api/ads/${ad.id}`, { method: "PUT", body: JSON.stringify(ad) }, currentUser.token);
      } else {
        await api("/api/ads", { method: "POST", body: JSON.stringify(ad) }, currentUser.token);
      }
      showToast("✅ Ad save ho gayi!");
      await loadAllAds();
      setModal(null);
    } catch (e) {
      showToast(e.message, "error");
  ...
  const deleteAd = async (id) => {
    setLoading(true);
    try {
      await api(`/api/ads/${id}`, { method: "DELETE" }, currentUser.token);
      await loadAllAds();
      showToast("🗑️ Ad hata di!");
      setModal(null);
    } catch (e) {
      showToast(e.message, "error");
    } finally {
  ...
  const toggleAd = async (id) => {
    try {
      await api(`/api/ads/${id}/toggle`, { method: "PUT" }, currentUser.token);
      await loadAllAds();
    } catch (e) {
      showToast(e.message, "error");
    }
  ...
  return (
    <div style={{ fontFamily:"'Poppins', sans-serif", minHeight:"100vh", background:"#FDF6EC", color:"#2C1810" }}>
      ...
      {currentUser && activeAds.length > 0 && (
        <div style={{ background:"#fff", borderBottom:"1px solid #F0E0C8", padding:"10px 16px" }}>
          <div style={{ maxWidth:800, margin:"0 auto" }}>
            <div key={adIndex} className="ada adb" style={{ background:`linear-gradient(135deg,${activeAds[adIndex % activeAds.length].bg_from || activeAds[adIndex % activeAds.length].bgFrom},${activeAds[adIndex % activeAds.length].bg_to || activeAds[adIndex % activeAds.length].bgTo})` }}
      ...
      {/* ══ LOADING BAR ══ */}
      {loading && <div style={{ position:"fixed", top:0, left:0, right:0, height:3, background:"linear-gradient(90deg,#FF6B00,#FFB300)", zIndex:999, animation:"sUp .3s" }} />}
          ...
          <div>
            ...
            {products.length === 0 ? (
              <div style={{ textAlign:"center", padding:60, color:"#888" }}>
                ...
                <p style={{ marginTop:8 }}>Products load ho rahe hain...</p>
              </div>
            ) : (
          ...
          <div>
            ...
            {!cart.length ? (
              ...
              <div style={{ background:"linear-gradient(135deg,#FFF8E1,#FFFDE7)", border:"2px solid #FFB300", borderRadius:16, padding:20, marginTop:8 }}>
                ...
                <button className="bg2" style={{ width:"100%", padding:14, fontSize:16 }} onClick={placeOrder} disabled={loading}>
                  {loading ? "⏳ Order ho raha hai..." : "✅ Order Karo & Commission Pao!"}
                </button>
              </div>
            </>}
          </div>
        ...
        {page==="dashboard" && currentUser && (
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:18 }}>
              {[
                { icon:"💰", val:`₹${currentUser.balance || 0}`, label:"Commission", clr:"#2E7D32" },
                { icon:"📦", val:userOrders.length, label:"Orders", clr:"#FF6B00" },
                { icon:"💳", val:`₹${currentUser.total_sales || currentUser.totalSales || 0}`, label:"Kul Bikri", clr:"#1565C0" },
              ...
              <div key={o.id} className="or">
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                  <div>
                    ...
                    <div style={{ fontSize:11, color:"#888" }}>{new Date(o.created_at).toLocaleDateString("hi-IN")}</div>
                  </div>
        ...
        {page==="admin" && (
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:10, marginBottom:18 }}>
              {[
                { icon:"👥", val:allUsers.length, label:"Salesmen", c:"#1565C0" },
                { icon:"📦", val:orders.length, label:"Orders", c:"#FF6B00" },
                { icon:"🛍️", val:products.length, label:"Products", c:"#2E7D32" },
                { icon:"💰", val:`₹${orders.reduce((s,o)=>s+(Number(o.commission)||0),0)}`, label:"Commission Diya", c:"#7B1FA2" },
              ...
              <div>
                ...
                {ads.map(ad => (
                  <div key={ad.id} style={{ borderRadius:14, overflow:"hidden", border:"1px solid #F0E0C8", marginBottom:12 }}>
                    <div style={{ background:`linear-gradient(135deg,${ad.bg_from||ad.bgFrom},${ad.bg_to||ad.bgTo})`, padding:"14px 16px", color:"white", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      <div>
                    ...
                    <div key={u.id} className="or">
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                        ...
                        <div style={{ textAlign:"right" }}>
                          ...
                          <div style={{ fontSize:11, color:"#888" }}>₹{u.total_sales||0} bikri</div>
                        </div>
                      </div>
                    </div>
                  ))
                  ...
                  : orders.map(o => (
                    <div key={o.id} className="or">
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                        ...
                        <span style={{ fontSize:11, color:"#888" }}>{new Date(o.created_at).toLocaleDateString("hi-IN")}</span>
                      </div>
  ...
  return (
    <div style={{ maxWidth:400, margin:"0 auto" }}>
      ...
      <div style={{ background:"white", borderRadius:20, padding:28, boxShadow:"0 8px 32px rgba(255,107,0,0.1)", border:"1px solid #F0E0C8" }}>
        ...
        <button className="bp" style={{ width:"100%", padding:14, fontSize:16 }} onClick={() => onLogin(phone, pass)} disabled={loading}>
...
function RegisterPage({ onRegister, showToast, onSwitch, loading }) {
  ...
  const verifyAndRegister = async () => {
    ...
    try {
      // Verify OTP
      const vRes = await fetch(`${API}/api/auth/verify-otp`, {
        method:"POST", headers:{"Content-Type":"application/json"},
      ...
      // Register
      await onRegister(name.trim(), phone, pass, vData.temp_token);
    } catch(e) {
      showToast(e.message, "error");
    } finally {
      setSending(false);
      ...
      <div style={{ background:"white", borderRadius:20, padding:28, boxShadow:"0 8px 32px rgba(46,125,50,0.1)", border:"1px solid #F0E0C8" }}>
        {step===1 && (
          <>
            ...
            <button className="bg2" style={{ width:"100%", padding:14, fontSize:15 }} onClick={sendOtp} disabled={sending}>
              {sending ? "⏳ OTP bhej raha hai..." : "📱 OTP Bhejo"}
            </button>
          </>
        )}
        {step===2 && (
          <>
            ...
            <button className="bg2" style={{ width:"100%", padding:14, fontSize:15 }} onClick={verifyAndRegister} disabled={sending || loading}>
              {sending || loading ? "⏳ Verify ho raha hai..." : "✅ Verify & Account Banao!"}
            </button>
          </>
        )}
        <p style={{ textAlign:"center", marginTop:16, fontSize:13, color:"#888" }}>
...
function AdModal({ ad, onSave, onDelete, onClose, loading }) {
  ...
  const handleSave = () => {
    onSave({ ...form, bg_from:form.bgFrom, bg_to:form.bgTo });
  };
  ...
  return (
    <div className="mb">
      ...
      <div style={{ marginBottom:14 }}>
        <label style={{ fontSize:12, fontWeight:600, color:"#666" }}>🔗 Click Link (optional)</label>
